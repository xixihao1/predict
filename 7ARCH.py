...
def ts_plot(residuals, stan_residuals, lags=40):
    residuals.plot(title='ARCH 模型残差图', figsize=(10, 5))
    plt.show()
    fig, ax = plt.subplots(nrows=1, ncols=2, figsize=(10, 5))
    ax[0].set_title('ARCH 标准残差密度图')
    ax[1].set_title('ARCH 标准残差概率图')
    residuals.plot(kind='kde', ax=ax[0])
    probplot(stan_residuals, dist='norm', plot=ax[1])
    plt.show()
    plot_acf(stan_residuals, lags=lags, title='ARCH 标准残差自相关性图')
    plt.show()
    plot_pacf(stan_residuals, lags=lags, title='ARCH 标准残差偏自相关性图')
    plt.show()

# 建立
garch = arch_model(df['data1'], vol='ARCH', p=1, q=1, dist='normal')
...
def gridsearch(data, p_rng, q_rng):#网格搜索函数`gridsearch()`，用于寻找最适合波动性的GARCH模型
    ...
    for p in range(len(p_rng)):
        for q in range(len(q_rng)):
            model = arch_model(data, vol='ARCH', p=p_rng[p], q=q_rng[q], dist='normal')
...
# 建模：使用最优的 p  q 值
garch = arch_model(df['data1'], vol='ARCH', p=17, q=25, dist='normal')
fgarch = garch.fit(disp='off')
resid = fgarch.resid
