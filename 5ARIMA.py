...
# 差分后的结果
D_data = data.diff().dropna()#使用`diff()`函数对数据进行一阶差分
D_data.columns = ['流量差分']
...
plt.show()
print('差分序列的ADF检验结果为：', ADF(D_data['流量差分']))  # 平稳性检测
# 返回值依次为adf、pvalue、usedlag、nobs、critical values、icbest、regresults、resstore

# 白噪声检验
print('差分序列的白噪声检验结果为：', acorr_ljungbox(D_data, lags=1))  # 返回统计量和p值,lags=1参数指定了要计算的滞后阶数。
